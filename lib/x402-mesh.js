...
function getPrivateKey() {
  ...
  if (!value) {
    throw new Error("X402_MESH_PRIVATE_KEY is not configured");
  }
...
function validateVendor(vendor) {
  ...
  const required = ["vendor_id", "name", "category", "endpoint", "public_key", "contact"];
  for (const field of required) {
    if (typeof vendor[field] !== "string" || vendor[field].length === 0) {
      throw new Error(`${field} is required`);
    }
  }
  if (!/^[a-z0-9][a-z0-9-_.]{1,63}$/.test(vendor.vendor_id)) {
    throw new Error("vendor_id must be a lowercase slug");
  }
...
function staticRegistry() {
  ...
  if (configured) {
    try { return JSON.parse(configured); } catch { throw new Error("X402_MESH_REGISTRY_JSON is invalid JSON"); }
  }
...
async function putRegistry(registry) {
  const stored = await kvCommand(["SET", REGISTRY_KEY, JSON.stringify(registry)]);
  if (stored === null && !env("UPSTASH_REDIS_REST_URL")) {
...
function buildMesh402({ vendorId, category, resource, price, requestId }) {
